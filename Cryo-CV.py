    ...
    def grad2(
        ...
        for k in range(number_of_nodes):
            ...
            gradx = (
                ...
                * np.exp(-G[k])
                * (
                    (1 / prob_matrix.shape[1])
                    * np.dot(Cv_differences[k, :, 0], prob_matrix[k])
                    - np.mean(prob_matrix, axis=1)[k, :]
                    * np.mean(Cv_differences[k], axis=0)[0]
                )
            )
            grady = (
                2
                ...
                * (
                    (1 / prob_matrix.shape[1])
                    * np.dot(Cv_differences[k, :, 1], prob_matrix[k])
                    - np.mean(prob_matrix, axis=1)[k, :]
                    * np.mean(Cv_differences[k], axis=0)[1]
                )
            )
...
for s in range(gradient_steps):
    ...
    for j in range(number_of_nodes):
        ...
        for i in range(sampling_steps):
            ...
            new_cv = (
                new_cv - grad_energy_funct * step_size + np.sqrt(2 * step_size) * noise
            )
    ...
    fe_prof, log_posterior = CryoBife.optimizer(
        CB, sim_path, images, Sample_per_nodes, sigma
    )
    ...
    for i in range(number_of_batches):
        ...
        Gra = CryoBife.grad2(
            sim_path, Sample_per_nodes, images_batch, fe_prof, sigma, nu, Cv_differences
        )
