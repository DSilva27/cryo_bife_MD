...
brownian_traj = brownian_motion.run_brownian_sim(
    initial_path, steps=100, step_size=0.001
)
...
plt.plot(
    brownian_path[:, 0], brownian_path[:, 1], ls="--", marker="o", label="before string"
)
plt.legend()
plt.show()
...
restrained_traj = euler_maruyama.run_euler_maruyama(
    brownian_path, harm_rest_energy_and_grad, harm_params, steps=100
)
